...
// Scatters `count` landmass centers (in grid-cell units) so N continents
// spread out across the whole canvas instead of clustering, jittered by a
// large fraction of each center's own ceil(sqrt(count)) x ceil(sqrt(count))
// partition (stratified sampling) so the result reads as organic scatter,
// not a rigid grid — every center still starts at its own partition's slot
// so a small N can't all wander into one corner. Deterministic from the
// seed, same reproducibility guarantee as everything else here.
//
...
// exact same size).
//
// The multiplier is floored at 0.4: below that, a continent's own area
// falls under generateTerrain's MIN_LANDMASS_AREA_FRACTION noise filter and
// gets silently culled, so low landmassScale (esp. combined with a higher
