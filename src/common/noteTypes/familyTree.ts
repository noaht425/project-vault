...
// FamilyTreeDiagram.tsx (confirmed with the user: blood/marriage ties must
// stay unambiguous from social ties) — but structurally they're handled
// identically here, since friend/rival/enemy/romantic are symmetric/mutual
// exactly like spouse/sibling already are (no hierarchy, no reverse phrasing
// needed, same dedupe-by-unordered-pair treatment).
...
// same "easy to find and adjust" convention settlementGenerator.ts uses for
// its own generation constants (AVG_HOUSEHOLD_SIZE, etc.).
 ...
 * typically sourced from real npc notes' optional `age` field via
 * `NoteRefApi.readFrontmatterByTitle`) — missing age data means the pair is
 * silently skipped, same "harmless when data's missing" pattern as a
 * dangling wiki-link or an unmatched calendar note elsewhere in this app.
 */
