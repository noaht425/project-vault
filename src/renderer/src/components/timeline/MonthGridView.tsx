...
// settlement note — same as EventSheet.tsx's identical local schema (only
// two call sites so far, not yet worth extracting).
 ...
 * year 1's first month/era — a calendar is useful to look at even completely empty. */
...
// Step 3 of docs/plans/2026-07-29-month-grid-campaign-date.md — a real
// "look at the month of Aucaela" page, distinct from both the calendar
// editor (defines structure) and the pill timeline (scatters everything
// chronologically across a zoomable axis). Anchored to a per-vault
// campaign "today" date, which also drives the upcoming-events list.
export function MonthGridView({ onOpenEvent }: { onOpenEvent: (path: string) => void }): React.JSX.Element {
  ...
  useEffect(() => {
    const load = async (): Promise<void> => {
      try {
        const [allEvents, calendarMatches, vaultSettings, locationMatches, settlementMatches] = await Promise.all([
  ...
  // Opens on the campaign date's month (or the latest event's, or a bare
  // year 1) the first time data finishes loading — never again after, so
  // paging months/switching calendars isn't fought by this effect re-firing.
  useEffect(() => {
  ...
  // Event chips are scoped to whichever calendar the grid is currently
  // showing (a day/month only means something relative to ONE calendar's
  // own structure) — recurring events are expanded bounded to this specific
  // month's range, not the whole vault's date span like the pill timeline.
  ...
  // Upcoming list spans EVERY calendar in the vault, same as the pill
  // timeline — canonical minutes are a shared epoch across all calendars
  // (calendarMath.ts's own design), so "what's next" doesn't need to be
  // scoped to just the grid's current calendar.
          ...
          return (
            ...
            >
              ...
              {dayEvents.slice(0, MAX_EVENT_CHIPS_PER_DAY).map((e, ei) => (
                <div key={`${e.path}-${ei}`} className="month-grid-event-chip">
          ...
          {(dayBuckets.get(selectedCell.day) ?? []).length > 0 && (
            <ul style={{ margin: '4px 0', paddingLeft: 18, fontSize: 13 }}>
              {(dayBuckets.get(selectedCell.day) ?? []).map((e, i) => (
                <li key={`${e.path}-${i}`}>
                  <button className="link-button" onClick={() => onOpenEvent(e.path)}>
                    {e.title}
                  </button>
                </li>
              ))}
      ...
      {settings?.campaignDate && (
        <div style={{ marginTop: 12 }}>
          ...
          {upcoming.length === 0 ? (
            ...
            <ul style={{ margin: '4px 0', paddingLeft: 18, fontSize: 13 }}>
              {upcoming.map((item, i) => (
                <li key={`${item.event.path}-${i}`}>
                  <button className="link-button" onClick={() => onOpenEvent(item.event.path)}>
