...
import { makeGrid, setTerrain, footprint, reachFt, terrainAt } from "../src/common/sim/battle/grid";
import {
  feetBetweenCells,
  feetBetweenBoxes,
  boxOf,
  ...
  coneCells,
  lineTemplateCells,
} from "../src/common/sim/battle/geometry";
import { reachable, pathToward, type MoveContext } from "../src/common/sim/battle/movement";
...
describe("fog of war — visibleCells", () => {
  it("a wall blocks sight past it", async () => {
    const { visibleCells } = await import("../src/common/sim/ui");
...
describe("measurement ruler helpers", () => {
  it("cellDistanceFt matches PHB 5-10-5", async () => {
    const { cellDistanceFt } = await import("../src/common/sim/ui");
  ...
  it("rulerLine returns a connected cell path including both endpoints", async () => {
    const { rulerLine } = await import("../src/common/sim/ui");
