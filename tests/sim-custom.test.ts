...
import {
  ...
  applyFeats,
  applyItems,
} from "../src/common/sim/ui";
import { runScenarioOnce, runScenario, standardParty, buildParty } from "../src/common/sim/engine/scenario";
import { parseCombatant, type Combatant } from "../src/common/sim/schema";
...
describe("per-PC spell picker", () => {
  it("replaces a caster's spell list with the chosen ids", async () => {
    const { buildParty } = await import("../src/common/sim/engine/scenario");
    const pc = buildParty([
  ...
  it("stamps caster metadata so an imported PC can be re-spelled", async () => {
    const { CASTER_BUILDERS } = await import("../src/common/sim/spells/casterTemplates");
...
describe("more races + flight", () => {
  it("Fairy: fly speed + Faerie Fire from level 3", async () => {
    const { applyRace } = await import("../src/common/sim/engine/pc-extras");
    const { makeTemplate } = await import("../src/common/sim/engine/templates");
  ...
  it("a flyer ignores difficult terrain in Battle mode", async () => {
    const { runBattle } = await import("../src/common/sim/battle");
    const { gridFromDef } = await import("../src/common/sim/battle/grid");
...
describe("spell picker — class resolution", () => {
  it("resolves cleric / artificer from a templateId even without stamped caster metadata", async () => {
    const { pcSpellClass } = await import("../src/common/sim/ui");
    const { applyPickedSpells } = await import("../src/common/sim/spells/pick");
    const { makeTemplate } = await import("../src/common/sim/engine/templates");
